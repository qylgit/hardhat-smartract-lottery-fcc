...
async function getSigner(hre, address) {
    const { CustomEthersSigner: SignerWithAddressImpl } = await Promise.resolve().then(() => __importStar(require("../signers")));
...
async function collectLibrariesAndLink(artifact, libraries) {
    ...
    for (const [linkedLibraryName, linkedLibraryAddress] of Object.entries(libraries)) {
        ...
        if (!ethers.isAddress(resolvedAddress)) {
            throw new plugins_1.NomicLabsHardhatPluginError(pluginName, `You tried to link the contract ${artifact.contractName} with the library ${linkedLibraryName}, but provided this invalid address: ${resolvedAddress}`);
        }
